...
const char * const BITCOIN_CONF_FILENAME = "techiecoin.conf";
const char * const BITCOIN_PID_FILENAME = "techiecoind.pid";
...
#else
    const char* pszModule = "techiecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Techiecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Techiecoin";
#else
    // Unix
    return pathRet / ".techiecoin";
#endif
#endif
}
