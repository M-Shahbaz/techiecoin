Techiecoin Core version *0.15.1* is now available from:

  <https://download.techiecoin.org/techiecoin-0.15.1/>
...
A number of changes to the way Techiecoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 techies per kB (0.00001 TCC). This is to prepare for dropping the minimum transaction fee to 0.1 techies per kB (0.0001 TCC) in the next release.
